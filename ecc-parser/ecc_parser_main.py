        ...
        while True:
            try:
               ...
               # print("select: r=%s w=%s e=%s" % (r,w,e))        # debug output to command line
                ...
                print(msg)
                siteid_upper = get_siteid(msg, debug)
                siteid =siteid_upper.lower()
                #print("upper case:%s lower case:%s",siteid_upper,siteid)
                #print("main siteid:",siteid)
                if(siteid == ""):
                    if(debug == True):
                        print("Invalid Site id:",siteid)
                    continue
                # Parse config file and find list of topics, parameters and no of sensors for a site
                topic_lst,param_list,noOfSensors = parse_config_file(siteid,configfile,debug)
                topic_list = topic_cleanup(topic_lst,debug)

                if(len(topic_lst) == 0 or len(param_list) == 0 ):
                    if(debug == True):
                        print("Topic list is empty",topic_list)
                        continue
                
                # Function to read cert.txt and return list of certificate files
                certFiles = get_certificates( debug)
               
                if(len(certFiles) == 0):
                    if(debug == True):
                        print("Certificate files list empty!")

                if(debug == True):
                    print("certificate files:",certFiles)

                mqttclientid = init_mqtt_interface(certFiles, mqtt_client,end_point, debug)    # Initialising mqtt interface to IoT core
                # if site id returned is keep that indicates it is gateway as gateway doesnot
                # have any specific site id
                if(msg[:4] == "keep"):
                    process_gateway_data(msg, siteid,topic_list,mqttclientid ,debug)
                else:
                    process_sensor_data(msg,noOfSensors,siteid,param_list,topic_list,mqttclientid, debug)
